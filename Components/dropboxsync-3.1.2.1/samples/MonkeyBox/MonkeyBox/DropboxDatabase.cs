...
namespace MonkeyBox
{
	...
	public static class DropboxHelper
	{
		...
		public static Monkey Update (this Monkey monkey, DBRecord record)
		{
			...
			monkey.Rotation = ((NSNumber)record.Fields ["Rotation"]).NFloatValue;
			monkey.Scale = ((NSNumber)record.Fields ["Scale"]).NFloatValue;
			monkey.X = ((NSNumber)record.Fields ["X"]).NFloatValue;
			monkey.Y =((NSNumber)record.Fields ["Y"]).NFloatValue;
			monkey.Z =((NSNumber)record.Fields ["Z"]).NIntValue;
			#else
			monkey.Name = record.Fields ["Name"].ToString ();
